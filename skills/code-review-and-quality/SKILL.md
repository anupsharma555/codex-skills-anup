...
In implementation mode, fix review issues immediately only when they are clearly inside the requested scope and the correction is low-risk. For real but out-of-scope issues, record them separately or use `repo-diagnostic-backlog-review` when the user wants a repo-level backlog. Do not silently expand the task into unrelated cleanup.
